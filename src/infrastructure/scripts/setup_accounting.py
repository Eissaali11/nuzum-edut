...
def add_accounting_to_app():
    ...
    if 'from src.routes.accounting import accounting_bp' not in app_content:
        # البحث عن موقع الإدراج
        lines = app_content.split('\n')
        insert_index = -1
        
        for i, line in enumerate(lines):
            if 'from src.routes.vehicle_operations import vehicle_operations_bp' in line:
                insert_index = i + 1
                break
        ...
        if insert_index > -1:
            ...
            lines.insert(insert_index + 2, "    from src.routes.accounting import accounting_bp")
            lines.insert(insert_index + 3, "    from src.routes.accounting_extended import accounting_ext_bp")
