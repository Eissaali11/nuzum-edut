"""
English Workshop PDF Report Generator
Comprehensive vehicle workshop reports in English to avoid Arabic text encoding issues
"""
...
def generate_workshop_pdf(vehicle, workshop_records):
    """Generate comprehensive workshop PDF report in English"""
    
    try:
        print("Starting English workshop PDF generation...")
        ...
        vehicle_info = [
            f"License Plate: {vehicle.plate_number}",
        ...
        if workshop_records and len(workshop_records) > 0:
            ...
            for record in workshop_records:
                ...
                technician = record.technician_name if record.technician_name else "Not specified"
            ...
            if record_count > 0:
                ...
                for record in workshop_records:
                    reason = get_reason_english(record.reason)
                    status = get_repair_status_english(record.repair_status)
                    workshop = record.workshop_name if record.workshop_name else "Not specified"
        ...
        print("English workshop PDF generated successfully!")
        return buffer.getvalue()
        
    except Exception as e:
        print(f"Error generating English workshop PDF: {str(e)}")
        raise e
